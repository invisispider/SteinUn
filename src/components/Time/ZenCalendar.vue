...
onMounted(() => {
  monthName.value = ref(
    coreDate.value.toLocaleString("en-US", { month: "long" })
  );
...
watchEffect(() => {
  ...
  state.daysInCurrentMonth = new Date(
    gYear.value,
    state.monthNum + 1,
    0
  ).getDate();
...
const selectHabit = (newHabit) => {
  ...
  coreDate.value = new Date(
    gYear.value,
    0,
    state.habitNum * daysInHabit.value + 1
  );
  state.dayStartOffset = store.gregDayNames.indexOf(state.startingDateName);
...
const changeDate = (d: number) => {
  coreDate.value = new Date(
    gYear.value,
    0,
    state.habitNum * daysInHabit.value + d
  );
  state.dayOfHabit = d;
...
<template>
  <div class="untable">
    <div class="calendars-container">
      <section id="habits">
        <h2>
          {{ dayOfWeekZen }} {{ state.dayOfHabit }} {{ habitName }}
          {{ 10000 + gYear }}
        </h2>
        <ol>
          <li
            v-for="hab in store.habitNames"
            @click="selectHabit(hab)"
            :class="hab == habitName ? 'selected' : ''"
          >
            {{ hab }}
          </li>
        </ol>
      ...
      <section id="months">
        ...
        <div class="unchunk-greg">
          <div
            class="baheader baheader-greg"
            v-for="gregDayName of store.gregDayNames"
            :key="gregDayName"
          >
            {{ gregDayName }}
          </div>
          <div
            v-for="index of state.dayStartOffset"
            class="uncell uncell-greg"
            :key="index"
          ></div>
          <div
            class="uncell uncell-greg"
            v-for="index of state.daysInCurrentMonth"
            :class="index == state.gregDate ? 'selected-date' : ''"
            :key="index"
          >
            {{ index }}
          </div>
