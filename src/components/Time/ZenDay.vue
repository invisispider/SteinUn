...
const hou_height = computed(() => Math.floor(svgHeight - (svgHeight / 24) * store.zhour))
const rotateOrigin = (h_h) => "transform-origin: "+gregWidth+"px "+h_h+"px"
</script>
<template>
  <div class="day-container">
    <svg :width="svgWidth" :height="svgHeight">
      <g id="gregMeter" fill="none">
        <template v-for="(num, i) in 25">
          <rect v-if="i < 6 || i > 17" :y="rSpread * i"
            class="greg-dark" :width="gregWidth" :height="svgHeight/24"
          />
          <rect v-else :y="rSpread * i" class="greg-light"
          :width="gregWidth" :height="svgHeight/24"
          />
          <text
          :x="svgWidth / 100"
      ...
      <g id="zenMeter" fill="none">
        <rect :x="gregWidth" :y="0" :width="halfWidth" :height="10*zSpread+zenSpread" class="zen-borders" />
        ...
        <template v-for="(zses, j) in store.zsessionNames" :key="j">
          <!-- <template v-for="k in 10" :key="k"> -->
            ...
            /> -->
            <rect :x="gregWidth" :y="zenSpread+zSpread*j-zSpread"
            :width="halfWidth" :height="zSpread" class="zen-borders"
            />
            <text :x="svgWidth/4" :y="j*zSpread+zSpread/2" class="zen-mint">
      ...
      <g id="clockHand" fill="none" >
        <line x1="0" :y1="hou_height" :x2="halfWidth+gregWidth"
        :y2="hou_height" class="clock-hand"
        />
        <circle :cx="gregWidth" :cy="hou_height" r="20"
          class="center-circle tick-pulse" :style="rotateOrigin(hou_height)" />
      </g>
    </svg>
  </div>
</template>
