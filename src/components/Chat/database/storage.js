...
export const getSoundUrl = async (fileName) =>
  await getDownloadURL(ref(storage, `${SFX_PATH}/${fileName}`));
export const getAvatarUrl = async (fileName) =>
  await getDownloadURL(ref(storage, `${AVATARS_PATH}/${fileName}`));
